...
class Game {
  ...
  coverListener() {
      ...
      var p = getEventPosition(theEvent);
        ...
        removeEvent(canvas, this.eventType.start, this.handler2); //点击之后就移除该按钮点击
		...
		addEvent(canvas, this.eventType.start, this.handler2);
  }

  //绘制得分
	drawScore(score, fontSize, left, top, color, align) {
		this.ctx.save();
		this.ctx.font = `Bold ${fontSize}px Arial`;
		this.ctx.textAlign = align || 'right';
  ...
  run() {
    ...
    //分数
    this.drawScore(this.score, 50, 20, 50, '#fff', 'left');
  ...
  gameOver() {
    ...
    setTimeout(() => {
      ...
      removeEvent(canvas, this.eventType.start, this.handler1);
      addEvent(canvas, this.eventType.start, this.handler2);
    }, 2000);
  }
  ...
  initListener() {
      ...
      this.bird.g = 1; //重置g
		...
		addEvent(canvas, this.eventType.start, this.handler1);
	}
